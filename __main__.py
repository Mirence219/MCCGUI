...
v0.7    新增功能：
        1.新增离线账户登录方式：可以使用离线账户登录服务器；
        2.更改正版账户登录方式：为保护用户数据，正版账户每次启动后需要跳转到浏览器进行登录，离线登录和第三方认证登录保持不变；
        3.新增账户高级设置：可以为假人账户设置更多的MCC配置信息，以便未来实现更多功能；
        4.添加和编辑账户时新增高级选项：可以配置游戏版本、游戏支持语言等信息（更多信息需要添加后在账户高级设置内设置）；
        5.新增彩色消息显示：监听窗口可以显示带颜色和样式的服务器消息，颜色与游戏内保持一致；√
        优化内容：
        1.优化代码结构，提高代码可读性；√
        2.改用数据库存储账户数据；
        3.密码将采用加密方式存储（无法保证绝对安全）；
        4.日志存储时将直接存储带ANSI转义字符的内容，以便重新打开控制窗口后读取日志文字的样式信息；√
        5.改用的专用的TOML解析工具设置MCC的配置文件；
        修复内容：
...
if __name__=="__main__":
    print(f"主进程MCCGUI_v{__version__} {"for MangoCraft" if MangoCraft else ""}（{os.getpid()}）已启动。")
    freeze_support()
    app = run()
    mainloop()
