...
v0.7    新增功能：
        1.部分登录方式优化：
            ...
            ②更改正版登录方式：为保护用户数据，正版账户首次启动后需要跳转到浏览器进行登录，后续启动可直接登录；√
        2.控制窗口优化：
            ①新增账户高级设置：可以为假人账户设置更多的MCC高级配置信息，以便未来实现更多功能；
            ②新增彩色消息显示：解析MCC的ANSI转义符，监听窗口可以正常显示大部分带颜色和样式的消息；√
            ③监听窗口背景颜色变化：监听窗口的背景颜色改为黑色，以更好地显示彩色信息；  √
        3.添加和编辑账户窗口优化：
            ①新增高级设置选项：可以指定游戏服务器和认证服务器的端口信息 √
            ②可以配置游戏版本、游戏支持语言等信息（更多信息需要添加后在账户高级设置内设置）；
            ③编辑账户时将不再显示先前的密码，若不保存的密码不变。 √
        优化内容：
        1.优化代码结构，提高代码可读性；√
        2.改用数据库存储账户数据；√
        3.账户密码采用加密方式存储（无法保证绝对安全）；
        ...
        8.删除账户后会将配置文件一同删除；  √
        9.为保护第三方账户的信息，每次关闭账户后会清空配置文件内密码部分的内容。 √
        10.每一次启动都会生成新的日志文件； √
        修复内容：
