...
## 📌 注意事项
1. 系统兼容：仅支持 Windows 系统；
2. 数据安全：第三方认证账户密码采用双向加密存储，建议定期更换密码，切勿泄露配置文件/数据库；

## 🚀 快速开始
1. 下载并解压 MCCGUI 发布包（暂时不公开分发，仅内部测试使用）；
...
### v0.7.1（最新）
#### 优化内容：
 1.优化新增、删除、编辑账户时刷新账户列表的逻辑；  
 2.优化了连通性测试，重复出现的IP只会进行一次测试，减少无效请求；
#### 漏洞修复：
 1.修复了新增、删除、编辑账户后原先已打开的控制窗口失效的问题；  
 2.修复游戏服务器连在通性测试完成后异常显示超时的问题；
...
4. 监听窗口升级为「控制窗口」：关闭窗口后仍后台监听，重新打开可恢复历史日志+窗口关闭期间的新内容；
